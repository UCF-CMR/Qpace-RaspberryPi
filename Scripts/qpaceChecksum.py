#! /usr/bin/env python
#qpaceChecksum.py by Minh Pham & Jonathan Kessluk
#05-18-2018, Rev. 1.2
...
def compareChecksum(input1, input2):
    ...
    try:
        ...
        if hasattr(input1, 'read') or isinstance(input1, str):
            input1 = checksum(turntofile(input1))
        elif isinstance(input1,bytes):
            input1 = checksum(input1)
        else:
            raise TypeError("Not string, file, or bytes.")

        if hasattr(input2, 'read') or isinstance(input2, str):
            input2 = checksum(turntofile(input2))
        elif isinstance(input2,bytes):
            input2 = checksum(input2)
        else:
            raise TypeError("Not string, file, or bytes.")
